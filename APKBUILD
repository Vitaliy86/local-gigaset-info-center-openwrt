...
pkgver="1.7"          # ← чистая версия, без -r0
pkgrel=0              # ← отдельная переменная для релиза
epoch=0
arch="noarch"
...
source="" # Указываем, что файлы берутся из репозитория напрямую
...
package() {
    ...
    install -m 0755 gigaset-info-center.init "$dest/etc/init.d/gigaset-info-center"
